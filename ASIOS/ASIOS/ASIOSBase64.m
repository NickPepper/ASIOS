...
@implementation ASIOSBase64

+ (NSString *)encodeBase64WithString:(NSString *)strData
{
    //return [AFStrings encodeBase64WithData:[strData dataUsingEncoding:NSUTF8StringEncoding]];
...
+ (NSData *)decodeBase64WithString:(NSString *)strBase64
{
    ...
    while ( ((intCurrent = *objPointer++) != '\0') && (intLength-- > 0) )
    {
        ...
        if (intCurrent == -1) {// we're at a whitespace -- simply skip over
            continue;
        } else if (intCurrent == -2) {// we're at an invalid character
            free(objResult);
            return nil;
        }
